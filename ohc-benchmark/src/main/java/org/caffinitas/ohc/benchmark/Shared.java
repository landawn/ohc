...
final class Shared {
    ...
    static void printStats(String title, boolean bucketHistogram, PrintStream csv) {
        ...
        for (Map.Entry<String, GCStats> gcStat : gcStats.entrySet()) {
            ...
            System.out.printf("     GC  %-15s : count: %8d    duration: %8dms (avg:%6.2fms)    cores: %d%n", gcStat.getKey(), count, duration, runtimeAvg,
                    gs.cores);
        }
    ...
    private static void dumpStats(MergeableTimer timer, String header, PrintStream csv) {
        ...
        double snapMax = snap.getMax() / NANOS_PER_MILLI;
        double snapMin = snap.getMin() / NANOS_PER_MILLI;
        ...
        System.out.printf(
                "     %-10s: one/five/fifteen/mean:  %.0f/%.0f/%.0f/%.0f%n" + "                 count:                  %10d %n"
                ...
                count,
                //
                snapMin, snapMax, snapMean, snapStdDev, snap75, snap95, snap98, snap99, snap999, snapMedian);

        if (csv != null) {
            csv.printf(";%d;%.0f;%.0f;%.0f;%.0f;%.5f;%.5f;%.5f;%.5f;%.5f;%.5f;%.5f;%.5f;%.5f;%.5f", count, oneMinuteRate, fiveMinuteRate, fifteenMinuteRate,
                    meanRate, snapMin, snapMax, snapMean, snapStdDev, snap75, snap95, snap98, snap99, snap999, snapMedian);
        }
    }
}
